...
void static commonInit(const uint8_t *cmdList) {
  ...
  GPIO_PORTA_PCTL_R = (GPIO_PORTA_PCTL_R&0x00FF0F00)+0x00000011;
  GPIO_PORTA_AMSEL_R &= ~0xCF;          // disable analog functionality on PA3,6,7
  TFT_CS = TFT_CS_LOW;
  RESET = RESET_HIGH;
  Delay1ms(500);
  RESET = RESET_LOW;
  ...
  // initialize SSI0
  GPIO_PORTA_AFSEL_R |= 0x2F;           // enable alt funct on PA2,3,5
  GPIO_PORTA_DEN_R |= 0x2F;             // enable digital I/O on PA2,3,5
                                        // configure PA2,3,5 as SSI
  GPIO_PORTA_PCTL_R = (GPIO_PORTA_PCTL_R&0xFF0F0000)+0x00202211;
  GPIO_PORTA_AMSEL_R &= ~0x2F;          // disable analog functionality on PA2,3,5
...
void ST7735_OutChar(char ch){
  ...
  ST7735_DrawCharS(StX*6,StY*10,ch,ST7735_YELLOW,ST7735_BLACK, 20);
  StX++;
  if(StX>20){
    StX = 20;
    ST7735_DrawCharS(StX*6,StY*10,'*',ST7735_RED,ST7735_BLACK, 20);
  }
  return;
}
//original code
void ST7735_OutChar2(char ch){
  if((ch == 10) || (ch == 13) || (ch == 27)){
    StY++; StX=0;
    if(StY>15){
      StY = 0;
  ...
  ST7735_DrawCharS(StX*6+10,StY*10 + 50,ch,ST7735_YELLOW,ST7735_BLACK, 3);
  StX = StX+4;
  if(StX>20){
    StX = 20;
    ST7735_DrawCharS(StX*6 + 10,StY*10 + 50,'*',ST7735_RED,ST7735_BLACK, 3);
  }
  return;
}
//********ST7735_OutString*****************
